...
// ==========================================================================
    ...
    /* Check to see if the Contact Number Question has been answered
     * Shows the next section if requirements are met
     */
    function contactNumberQuestionAnswered(section) {
        section.find("input[type=text]").keyup(function () {
            var $this = $(this);
            var $inputText = $this.val().replace(/\s+/g, '');
            if ($inputText.indexOf('1234567')>-1){
                //$('.changeContactNumber').hide(); // default blue button which is not wanted here
                $('#number-warning').html('No numbers containing 1234567 please').css({'font-weight':'bold', 'color':'#57a946', 'margin-top':'5px'});
                slideUpSection(section.next());
            }
            else if ($inputText.length >= 6) {
                //ajax send the abandoned cart timer here
                //my.viewModel.userName.subscribe();
                //$('.changeContactNumber').hide(); // default blue button which is not wanted here
                $('#number-warning').html('');
                showSection(section, "completedSection");
                showSection(section.next(), "openSection");
                if ( $('.innerPrimaryResidenceSectionNo').css('display') == 'none' ){ //new attempt to stop bouncing
                    slideDownSection(section.next());
                }
            }
            $('.changeContactNumber').hide(); // default blue button which is not wanted here
            if ($this.is(':focus')) {
            ...
            data: {icp: icpNumber[0]}
                ...
                .done(function (data) {
                    ...
                    if ($networkProvider === "") {
                        ...
                        $("span.network-summary").html("Your Network Company is " + $networkProvider);
    ...
    function getMonthsThresholds(billValue, billMonth) {
        ...
        $.get($('#Form_SignUpForm_Suburb').data('api-url') + 'getMonthsThresholds', function (data) {
            ...
            if ($bill <= $monthThreshhold) {
                $apiSelectedHidden.val("Low User - Must be a primary residence");
            }
            else {
...
//    }
    ...
    $(function () {
        ...
        contactNumberQuestionAnswered($questionFirst.next()); //mod for new early capture phone number
...
//        }
        ...
        $('#Form_SignUpForm_Application_IAcceptTheTermsAndConditions_Yes').attr('checked', true);
        ...
        $('.innerPrimaryResidenceSectionNo').hide();
        $('.innerPrimaryResidenceSectionYes').hide();
...
//                        console.log($nextSection);
            ...
            // Checks for Primary residence
            if ($optionset.is('#Form_SignUpForm_PrimaryResidenceDeclaration')) {
                var $cSection = $section.find(".innerSectionEU");
...
// for ref     function showSection(section, addSectionClass, addHover, dontShow) {
                        slideDownSection($cSection);
                        //first attempt at getting control of animation
                        //slideUpSection($('.innerPrimaryResidenceExplanation'));

                        if ( $('.innerPrimaryResidenceSectionNo').css('display') == 'block' ){
                            slideUpSection($('.innerPrimaryResidenceSectionNo'));
                        }
                       // if ( $('.innerPrimaryResidenceSectionYes').css('display') == 'none' ){
                            //slideDownSection($('.innerPrimaryResidenceSectionYes'));
                       // }

                        //showSection($section, "completedSection");
                        slideUpSection($('.pricePlanSection'));
                        disableContinue(2);

                    }
                    else {

                       //showSection($('.electricityUseSection'), "openSection");  //this may seem nuts to show it then slide it up, but otherwise this secrtion will not be available for use in the summary
                        //slideUpSection($('.electricityUseSection'));
                        showSection($section, "completedSection");
                        slideUpSection($cSection);

                        //slideUpSection($('.innerPrimaryResidenceExplanation'));

                        if ( $('.innerPrimaryResidenceSectionNo').css('display') == 'none' ){
                            slideDownSection($('.innerPrimaryResidenceSectionNo'));
                        }
                        if ( $('.innerPrimaryResidenceSectionYes').css('display') == 'block' ){
                            slideUpSection($('.innerPrimaryResidenceSectionYes'));
                        }

                        slideDownSection($('.pricePlanSection'));
                        $("#Form_SignUpForm_UserTypeAPISelected").val("Standard User");
                        enableContinue(2); // set to standard user
                    }
                });
...
//            }
        ...
        if ($section.hasClass('electricityUseSection')) {
...
//                console.log("A: " + $billValue.val() + " B: " + $billMonth.val());
                if ($billValueNumber > 0 && $billMonth.val() !== '') {      //} "placeholder") {
                    ...
                    slideDownSection($nextSection);
                    showSection($nextSection, "openSection"); //because its height will have been reduced to zero by slideupsection in previous question
...
//            });
    ...
    $(".icpSubmit").click(function (e) {
        ...
        if ($("#Form_SignUpForm_ICP_Number").val() !== "") {
            ...
            icpnumtext = $icpNum.replace(/[-' ']+/g, '');
            $icp = new Array($icpNum.replace(/[-' ']+/g, ''));
            //console.log($icp);
...
/*http://stackoverflow.com/questions/12061816/customizing-serializearray-to-a-different-array-format*/
...
function pollCartRecovery() {

    $('.customerContactNumber').find("input[type=text]").each(function () {
        var $this = $(this);
        var data = JSON.stringify(serializeObject($('form').serializeArray()));
        //console.log(data);
        if ($this.val().length >= 6) {
            //console.log('showsection - text quest blur 192');
            $.ajax({
                url: $('#Form_SignUpForm_Suburb').data('api-url') + 'cartRecovery',
                type: 'POST',
                dataType: 'json',
                // data: {
                data: {
                    ...
                    customername: 'kk',
                    contactnumber: '1234',
                    cartrecover: getCookie('CartRecover')
                }
            })
                    .done(function (data) {
                        // console.log("success");
            ...
            // $this.trigger("change"); // update knockout observable when focus lost to other applications
            //console.log(section);
