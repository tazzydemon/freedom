...
class SignUp extends UserDefinedForm {
...
class SignUp_Controller extends Page_Controller {
    private static $allowed_actions = array(
            'SignUpForm', 'doSignUp', 'doPhoneSubmit', 'Submitted', 'getICPNumber', 'getSchool', 'submit', 'getNetworkProvider', 'getMonthsThresholds', 'cartRecovery'
    );
    ...
    public function init() {
        ...
        if (empty($routing['Action'])) {
            ...
            Requirements::javascript($this->ThemeDir() . '/js/thirdparty/knockout-3.2.0.js');
    ...
    */
    public function SignUpForm() {
        ...
        if (!isset($_COOKIE['CartRecover'])) setcookie('CartRecover', uniqid());
        //ChromePhp::log(PHP_SESSION_DISABLED.PHP_SESSION_NONE.PHP_SESSION_ACTIVE.$me);
        require_once('FirePHPCore/fb.php');
        $request = $this->getRequest();
       // $declaredLocation = (string)$request['loc'];
        Session::set('declared_location', strtolower((string)$request['loc']));
        $freedomOnline = true;
        //var_dump($_SESSION);
        ...
        if ($this->Content != '') {
            $firstTabRightTitle = SignUpTab1::_tPlain('Intro , Price Check', '<p>Thank you for checking out our Freedom Plans!</p><p>If you choose not to join Pulse Energy, we will not keep your details on record.</p>');
        }
        else {
        ...
        $fields = FieldList::create(
                OurTabSet::create(
                        ...
                        Tab::create(
                                ...
                                FieldGroup::create(
                                        TextField::create('CustomerContactNumber', 'Your Contact Number')
                                                //->setRightTitle($firstTabRightTitle)
                                                ->setAttribute('placeholder', 'eg. 021 1234567 ')
                                                ->setAttribute('data-bind', "value: userContactNumber, visible: editingContactNumber && !userContactNumber(), hasFocus: editingContactNumber")
                                                ->setSmallFieldHolderTemplate("CustomerContactNumber_holder_small")
                                )->addExtraClass('openSectionNot customerContactNumber'),
                                FieldGroup::create(
                                ...
                                FieldGroup::create(
                                        TextField::create('Suburb', SignUpTab1::_tPlain('Address,ICP Screen - Q2 title', 'What is your address?'))
                                                ...
                                                ->setLeftTitle(SignUpTab1::_tPlain('Address,ICP Screen - Address Instructions', 'Please choose from one of the selections that appear after you start typing'))
...
//                                                ->setRightTitle('Does this ICP match the property you were searching for? Click <a href=\'/sign-up/how-to-find-your-icp\' target=\'_blank\'>here</a> or <a href=\'#\' data-reveal-id=\'myICP\'\ data-reveal>here</a> if you need to locate your ICP number on your current bill')
                                ...
                                FieldGroup::create(
                                        LiteralField::create('Business Message',
                                                SignUpTab1::_tPlain('ICP , Business Message', '<label class=\"left\"></label><p><b>This ICP appears to belong be a non-residential property. Please fill out the form below and we will be in contact with you.</b></p>')
                                        )
                                )
                                ...
                                FieldGroup::create(
                                        LiteralField::create('Can\'t Find ICP',
                                                SignUpTab1::_t('ICP , Can\'t Find', '<label class=\"left\">Let\'s try again</label><p>We could not automatically find your ICP from the address you have provided above.</p>')
                                        )
                                )
...
//                                                ->setSmallFieldHolderTemplate('FormField_holder_small')
                        ...
                        Tab::create(
                                ...
                                FieldGroup::create(
                                        AdvOptionsetField::Create('PrimaryResidenceDeclaration', SignUpTab2::_tPlain('Electricity use,pricing - Declaration', 'Is this a primary residence?'), array(
                                                'Yes' => 'Yes this is a primary residence',
                                                'No' => 'No this isn\'t a primary residence'
                                        ))
                                                ->addExtraClass('widthFixedLarge')
                                                ->addExtraClass('tagsClear')
                                                ->addExtraClass('optionset')
                                                ->setRightTitle(SignUpTab2::_tPlain('Primary Residence Explanation', 'A primary residence is the main place that someone lives. This cannot be a holiday home or bach and you can only have one primary residence.'))
                                                ->setAttribute('data-bind', 'checked: primaryResidenceDeclaration'),
//                                        FieldGroup::create(
//                                               LiteralField::create('ResidenceExplanation',
//                                                      SignUpTab2::_t('Primary Residence Explanation', '<label class=\"left\">What is a Primary Residence?</label><p><b>Not sure what text to put in here.</b></p>')
//                                               )->addExtraClass('residenceExplanation')
//                                        )->addExtraClass('innerPrimaryResidenceExplanation')
//                                         ->setSmallFieldHolderTemplate('FieldGroupResidence_holder'),

                                        FieldGroup::create(
                                               LiteralField::create('ResidenceMessageYes',
                                                      SignUpTab2::_t('Primary Residence Message, Yes', '<label class=\"left\">Primary Residence</label><p><b>Depending on the outcome of the following price question, <em>Low User – must be a Primary Residence</em> or <em>Standard User</em> will appear in the Plan Summary.</b></p>')
                                               )->addExtraClass('residenceMessageYes')
                                        )->addExtraClass('innerPrimaryResidenceSectionYes')
                                         ->setSmallFieldHolderTemplate('FieldGroupResidence_holder'),
                                        FieldGroup::create(
                                               LiteralField::create('ResidenceMessageNo',
                                                      SignUpTab2::_t('Primary Residence Message, No', '<label class=\"left\">Not a Primary Residence</label><p><b>As this is not a primary residence, the correct price plan for this property is a Standard User Plan.</b></p>')
                                               )->addExtraClass('residenceMessageYes')
                                        )->addExtraClass('innerPrimaryResidenceSectionNo')
                                         ->setSmallFieldHolderTemplate('FieldGroupResidence_holder'),

//                                        FieldGroup::create(
                                        //sample using external template
//                                                LiteralField::create('ResidenceMessageYes', SSViewer::execute_template(SSViewer::getTemplateFileByType('PrimaryResidenceBlock', 'Form_Content'),
//                                                                new ArrayData(array(
//                                                                        'Content' => SignUpTab2::_tPlain('Primary Residence Message, Yes', '<label class=\"left\"></label><p><b>Depending on the outcome of the following price question, Low User – must be a primary residence or Standard User will appear in the Summary of Plan.</b></p>'),
//                                                                        'Title' => SignUpTab2::_tPlain('Primary Residence Message Title, Yes', '<label class=\"left\"></label><p><b>This is a Primary Residence.</b></p>')
//                                                                )))
//                                                )->addExtraClass('residenceMessageYes')
//                                        )->addExtraClass('innerPreSection')->setSmallFieldHolderTemplate('FieldGroup_holder'),
                                        ...
                                        FieldGroup::create(SignUpTab2::_tPlain('Electricity use,pricing - Q1 title', 'Electricity Use'),
                                                ...
                                                TextField::create('MonthBillReceived', '') //,date("M", strtotime("-1 month"))
                                                        ->setAttribute('placeholder', 'Choose the month the bill was received')
                                                        ->setDescription('Month Bill Received')
                                                        ->addExtraClass('dropdown billMonth'),
                                                HiddenField::create('UserTypeAPISelected')->setTitle(''),
                                                HiddenField::create('UserTypeUserSelected')->setTitle('')
                                        )//->addExtraClass('openSection')
                                                ->setSmallFieldHolderTemplate('FieldGroupResidence_Holder')// override the composite detection that it seems to be getting. Smallfieldholder is used in a fieldgroup withing a fieldgroup
                                                ->addExtraClass('textFieldsOnly')
                                                //->addExtraClass('electricityUseSection')
                                                ->addExtraClass('innerSectionEU-not')
                                        ...
                                        ->addExtraClass('electricityUseSection')
                                        ->setFieldHolderTemplate("MultiBlockEU_FieldGroup_holder")
                                ,
                                FieldGroup::create(
                                        $PricePlan = AdvOptionsetField::create('PricePlan', SignUpTab2::_tPlain('Electricity use,pricing - Q2 title', 'Your Price Plan'), array(
                                                ...
                                                ->setDescription(SignUpTab2::_t('Electricity use,pricing - Q2 Description', 'Depending on the type of meter installed in your home, you may have a flat price for all energy used in a day or have a day/night price.')),
                        ...
                        Tab::create(
                                ...
                                FieldGroup::create(SignUpTab2::_tPlain('Gas use,Gas title', 'Gas with Pulse Energy'),

                                        AdvOptionsetField::Create('PulseGas', '', array(
                                                'Natural Gas' => 'Yes, I would like you to contact me about Natural Gas',
                                                'LPG' => 'Yes, I would like you to contact me about LPG',
                                                'No Gas Required' => 'No I don\'t need  gas'
                                        ))
                                                ->addExtraClass('widthFixedLarge')
                                                ->addExtraClass('tagsClear')
                                                ->addExtraClass('optionset')
                                                ...
                                                ->setAttribute('data-bind', 'checked: chooseGasChoice')
                                )->addExtraClass('openSection')
                                        ->setDescription(SignUpTab2::_tPlain('Gas use, description', 'By choosing gas with Pulse Energy you will enjoy the convenience of a single bill and our All Energy Discount'))
                                        ->setLeftTitle(SignUpTab2::_tPlain('Gas use, Choose Type', 'Select the gas service you require'))
                                        ->setAttribute('data-bind', ''),
...
//                        )
        ...
        foreach ($this->Fields() as $fld) {
            ...
            // DATEFIELD
            if ($rendered_field->Type() == 'date-alt text' || $rendered_field->Type() == 'date text' || $rendered_field->Type() == 'dateNZ text') {
                //new mod
...
//ChromePhp::log($validator);
    ...
    public function doSignUp($data, $form) {
        ...
        /* First of all lets deal to the cart recovery tracking. If we have got this far there is no need to keep it any more */
        $cartRecover = Cookie::get("CartRecover");
        if (!empty($cartRecover)) {
            $cartTrack = CartRecovery::get()->filter(array('UniqueID' => $cartRecover))->first(); // first() to change datalaist to object
            if (!$cartTrack->isEmpty()) {
                $cartTrack->update(array(
                    //UniqueID is NIT updated. This is the applications private session track
                        'CartComplete' => true,
                ));
                $cartTrack->write();
                //maybe delete the tracker instead
            }
...
//        var_dump($clientMeterString);
        ...
        if (empty($data_toSave['Cancellation_Reason'])) {
            ...
            $adminEmailBody = "<style type=\"text/css\">table, h1{font-family:arial, helevetica, sans-serif}</style>"
                    . "<h1>" . SignUpTab1::_tPlain('Product Name', 'Pulse Energy') . " Online Application " . $record_number . "</h1>"
...
//                        . $data_toSave['Application_dob_Month'] . "/"
                    ...
                    . "<tr><td>Primary Residence</td><td>" .$data_toSave['PrimaryResidenceDeclaration']. "</td></tr>"
...
//                        . $data_toSave['Application_DOB_Alternative_Month'] . "/"
                    ...
                    . "<tr><td>Gas Choice</td><td>" . $data_toSave['PulseGas'] . "</td></tr>"
...
//                          . "<tr><td>Moving Date</td><td>" . $data_toSave['Application_move_in_date_Day'] . "/" . $data_toSave['Application_move_in_date_Month'] . "/" . $data_toSave['Application_move_in_date_Year'] . "</td></tr>"
            ...
            $adminEmail = new Email('signup@pulseenergy.co.nz', $signupEmail, 'Pulse Energy Freedom Online Application', $adminEmailBody);
            ...
            $customerEmail->setBcc('julian.warren@pulseenergy.co.nz');
            ...
            $adminEmailBody = "<style type=\"text/css\">table, h1, h2{font-family:arial, helevetica, sans-serif}</style>"
                    . "<h1>" . SignUpTab1::_tPlain('Product Name', 'Pulse Energy') . " Online Application " . $record_number . "</h1></h1><br><h2>Incomplete Application</h2><br>"
                    . "<table cellpadding=\"5\" border=\"1\">"
                    . "<tr><td><b>Item</b></td><td><b>Value</b></td></tr>"
            ...
            $adminEmail = new Email('signup@pulseenergy.co.nz', $failEmail, SignUpTab1::_tPlain('Product Name', 'Pulse Energy') . ' Application Fail', $adminEmailBody);
    ...
    public function SubmissionTitle() {
        ...
        Session::clear('customer_name');
        return "Welcome to " . SignUpTab1::_tPlain('Product Name', 'Pulse Energy') . " " . $usr_name . "!";
    }
...
//        fb($decrequest);
        ...
        //Nelson is Nelson & Bay
        //Plus Tasman is often Nelson & Bay
        //Timaru and Oamaru ARE NOT a Region. One is in S Canterbury and the other in Otago so now we cant look for Otago or Canterbury
        if (strpos($region, 'Manawatu') !== false
                || strpos($region, 'Wanganui') !== false
                || strpos($region, 'Waikato') !== false
                || strpos($region, 'Nelson') !== false
                || strpos($region, 'Tasman') !== false
                || strpos($region, 'Canterbury') !== false
                || strpos($region, 'Canterbury') !== false
...
//        fb($icp_records['result'][$finalChoice]);fb($icp_records['address'][$finalChoice]);
    ...
    public function getNetworkProvider() {
        ...
        if ($icpN = $request->postVar('icp')) {
            ...
            //fb($icpDetails->icpDetails_v1Result->myMeteringHistory->allMeteringInstallations);
                ...
                foreach ($regMeters as $MeteringInstallation) {
                    ...
                    foreach ($MeteringInstallation as $allMeteringComponentsObject) {
                        ...
                        foreach ($allMeteringComponentsObject->allMeteringComponents->MeteringComponent as $MeteringComponentObject) {
                            ...
                            //fb($MeteringComponentObject->allMeteringChannels);
                            if (isset($MeteringComponentObject->allMeteringChannels)) {
                                ...
                                foreach ($MeteringComponentObject->allMeteringChannels->MeteringChannel as $MeteringChannel) {
                                    ...
                                    if (isset($MeteringChannel->settlementIndicator) && ($MeteringChannel->settlementIndicator == 'true')) {
                                        ...
                                        // fb($icpDetails->icpDetails_v1Result->myPricingHistory->allPriceCategoryCodes->PriceCategoryCode[0]->code);
                                        $GXPData = array(
...
//fb($MeteringChannel->vRegisterContentCode . $MeteringChannel->periodOfAvailability);
//fb($icpDetails->icpDetails_v1Result->myNetworkHistory->myNetwork->code);
//fb($icpDetails->icpDetails_v1Result->myPricingHistory->allPriceCategoryCodes->PriceCategoryCode[0]->code);
//fb($icpDetails->icpDetails_v1Result->myNetworkHistory->bus);
                                        if (!$live) {
                                            fb('$GXPData');
                                            fb($GXPData);
                                        }
                                        ...
                                        foreach ($PDFs as $PDF) {
                                            ...
                                            switch ($PDF->RegisterMap) {
                                                case 'I':
                                                    if (file_exists(__DIR__ . '/../../assets/Uploads/pricing-pdf/' . $PDFName . 'I.pdf')) {
                                                        $PDFName .= "I";
                                                    }
                                                    else if (file_exists(__DIR__ . '/../../assets/Uploads/pricing-pdf/' . $PDFName . 'UN.pdf')) {
                                                        $PDFName .= "UN";
                                                    }
                                                    else {
                                                        $PDFName .= "U";
                                                ...
                                                case 'D':
                                                    ...
                                                    break;
                                                case 'N':
                                                    if ($RegContentData['RegContentCode'] == 'N8') {
                                                        $PDFName .= "DT";
                                                    }
                                                    else {
                                                        $PDFName .= "UN";
                                                ...
                                                case 'T':
                                                    ...
                                                    break;
                                                case 'U':
                                                    if (file_exists(__DIR__ . '/../../assets/Uploads/pricing-pdf/' . $PDFName . 'UN.pdf')) {
                                                        $PDFName .= "UN";
                                                    }
                                                    else {
                                                        $PDFName .= "U";
                                                ...
                                                case 'E':
                                                    ...
                                                    break;
                                                case 'C':
                                                    if (file_exists(__DIR__ . '/../../assets/Uploads/pricing-pdf/' . $PDFName . 'C.pdf')) {
                                                        $PDFName .= "C";
                                                    }
                                                    else if (file_exists(__DIR__ . '/../../assets/Uploads/pricing-pdf/' . $PDFName . 'UN.pdf')) {
                                                        $PDFName .= "UN";
            ...
            // fb('no longer filtered array see line 859 Signup.php');
            // fb($meterDetailArray);
            if ($regMeters == true) {
...
//
    ...
    function cartRecovery() {
        $request = $this->getRequest();
        $decrequest = json_decode($request->postVar("json_data"), false);
        $cartRecover = Cookie::get("CartRecover");
//        ChromePhp::log($cartRecover);
//        ChromePhp::log($decrequest);
        //$recover = new CartRecovery();
        if (!empty($cartRecover)) {
            $cartTrack = CartRecovery::get()->filter(array('UniqueID' => $cartRecover))->first(); // first() to change datalaist to object
//            ChromePhp::log($cartTrack);
            if (empty($cartTrack) || $cartTrack->isEmpty()) { //second one for empty ss object
                $recover = new CartRecovery();
                $recover->UniqueID = $cartRecover;
                $recover->CustomerName = $decrequest->CustomerName;
                $recover->CustomerContactNumber = $decrequest->CustomerContactNumber;
                $recover->CustomerJSON = $request->postVar("json_data");
                $recover->write();
            }
            else {
                $cartTrack->update(array(
                    //UniqueID is NIT updated. This is the applications private session track
                        'CustomerName' => $decrequest->CustomerName,
                        'CustomerContactNumber' => $decrequest->CustomerContactNumber,
                        'CustomerJSON' => $request->postVar("json_data"),
                ));
                $cartTrack->write();
            }
...
class AbandonedCartCheck extends BuildTask {

    public $description = "To be executed as a CRON task. Checks to see if any CartRecovery database entries are older than a specified number if minutes and send an alert if they are.";

    public function run($request) {

        /* First of all lets deal to the cart recovery tracking. If we have got this far there is no need to keep it any more */
        $cartTrackList = CartRecovery::get();

        foreach ($cartTrackList as $cartTrack) {

            $lastAccessDate = new DateTime($cartTrack->LastEdited);
            $now = new DateTime();
            //var_dump($now);
            $lastAccessDate->modify('+10 min');
            //var_dump($now > $lastAccessDate);
            $interval = $lastAccessDate->diff($now);
            echo "invert " . $interval->invert . "\n";
            echo $interval->y . ' ';
            echo $interval->m . ' ';
            echo $interval->d . ' ';
            echo $interval->h . ' ';
            echo $interval->i . ' ';
            echo $interval->s . "\n";
            if (!$interval->invert && !$cartTrack->EmailSent && !$cartTrack->CartComplete) {
                $cartTrack->update(array(
                    //UniqueID is NIT updated. This is the applications private session track
                        'EmailSent' => true,
                ));
                ...
                $recoveryEmail = Config::inst()->get('SignUp', 'recoveryemail');

                //echo $recoveryEmail."\n";

                $data_toSave = json_decode($cartTrack->CustomerJSON, true);

                $adminEmailBody = "<style type=\"text/css\">table, h1{font-family:arial, helevetica, sans-serif}</style>"
                        . "<h1>Cart Recovery Warning Email</h1>"
                        . "<table cellpadding=\"5\" border=\"1\">"
                        . "<tr><td><b>Item</b></td><td><b>Value</b></td></tr>"
                        . "<tr><td colspan=2><h2>Basic Details </h2></td></tr>"
                        . "<tr><td>Customer Name</td><td>" . $data_toSave['CustomerName'] . "</td></tr>"
                        . "<tr><td>Customer Contact Number</td><td>" . $data_toSave['CustomerContactNumber'] . "</td></tr>"
                        . "<tr><td colspan=2>&nbsp;</td></tr>"
                        . "<tr><td colspan=2><h2>Extra Details if Available</h2></td></tr>"
                        . "<tr><td>Title</td><td>" . $data_toSave['Application_Title'] . "</td></tr>"
                        . "<tr><td>ICP Number</td><td>" . $data_toSave['ICP_Number'] . "</td></tr>"
                        ...
                        . "</td></tr>"
                        . "<tr><td>Price Plan</td><td>" . $data_toSave['PricePlan'] . "</td></tr>"
                        . "<tr><td>Gas Choice</td><td>" . (!empty($data_toSave['PulseGas']) ? $data_toSave['PulseGas'] : '') . "</td></tr>"
                        . "<tr><td>Smart meter</td><td>" . $data_toSave['SmartMeter'] . "</td></tr>"
                        . "<tr><td>Network Provider</td><td>" . $data_toSave['Network_Provider'] . "</td></tr>"
//                        . "<tr><td>School</td><td>" . $data_toSave['School'] . "</td></tr>"
//                        . "<tr><td>School Name</td><td>" . $data_toSave['SchoolName'] . "</td></tr>"
                ...
                $adminEmail = new Email('signup@pulseenergy.co.nz', $recoveryEmail, 'Cart Recovery: Pulse Energy Application', $adminEmailBody);
                $adminEmail->setCc('julian@bitstream.co.nz');
                $adminEmail->send();
